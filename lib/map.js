'use strict';
var specialKeys = require('./special-keys');
...
var create = module.exports = function () {
  ...
  function mapSequence(mode, from, to) {
    ...
    chars.forEach(function (c, index, arr) {
      current[c] = index < (arr.length - 1) ? {} : to;
      current = current[c];
    });
  }
  ...
  function map(mode, from, to) {
    ...
    if (from.length > 1) { 
      if (~specialKeys.indexOf(from)) return mapModifier(mode, from, to);
      if (mode !== 'insert')
        throw new Error('You can only map key sequences in insert mode and special keys or keys combined with modifiers like ctrl in insert or normal mode');

      return mapSequence(mode, from, to);
    }
  }
